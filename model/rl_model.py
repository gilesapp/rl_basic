...
class DQN(bModel):
    def __init__(self, *args, **kw):
        ...
        # replay memory
        self.memory = ReplayBuffer(self.BUFFER_SIZE, self.BATCH_SIZE, self.random_seed, self.device)
...
class DDPG(bModel):
    def __init__(self, *args, 
        ...
        # Noise process
        self.noise = OUNoise((1, self.action_size), self.random_seed)
    ...
    def act(self, state):
        ...
        action = torch.clip(action, -1, 1).numpy()
        action = self.wrap_action(action)
    ...
    def learn(self, experiences, gamma):
        ...
        with torch.no_grad():
            ...
            Q_targets_next = self.critic_target(next_states, self.wrap_action(actions_next))
        ...
        actor_loss = -self.critic_local(states, self.wrap_action(actions_pred)).mean()
...
class SAC(bModel):
    ...
    def learn(self, experiences, gamma):
        """Update policy and value parameters using given batch of experience tuples.
        Q_targets = r + γ * critic_target(next_state, actor(next_state))
        where:
            actor(state) -> action
            critic_target(state, action) -> Q-value)
        Params
        ...
        # ----- Critic loss -----
        with torch.no_grad():
            actions_next, log_pi_next = self.actor(next_states)
            Q_targets_next = self.critic_tgt(next_states, self.wrap_action(actions_next)) - self.alpha * log_pi_next
            Q_targets = rewards.unsqueeze(1) + (gamma * Q_targets_next * (1 - dones.unsqueeze(1)))
        ...
        Q_expected = self.critic(states, self.wrap_action(actions_pred))
...
class PPO(bModel):
    ...
    def act(self, state):
        ...
        action = action.cpu().data
        action = torch.clip(action[0], -1, 1).numpy()
        action = self.wrap_action(action)
